...
/* PRIVATE FUNCTIONS */
static uint64_t __default_hash(const uint64_t key);
static int __get_index(SimpleSet *set, const uint64_t key, uint64_t hash, uint64_t *index);
static int __assign_node(SimpleSet *set, const uint64_t key, uint64_t hash, uint64_t index);
static void __free_index(SimpleSet *set, uint64_t index);
static int __set_contains(SimpleSet *set, const uint64_t key, uint64_t hash);
static int __set_add(SimpleSet *set, const uint64_t key, uint64_t hash);
...
uint64_t* set_to_array(SimpleSet *set, uint64_t *size) {
    *size = set->used_nodes;
    uint64_t* results = (uint64_t *)calloc(set->used_nodes + 1, sizeof(uint64_t ));
    uint64_t i, j = 0;
    for (i = 0; i < set->number_nodes; ++i) {
        if (set->nodes[i] != NULL) {
...
static int __get_index(SimpleSet *set, const uint64_t key, uint64_t hash, uint64_t *index) {
    ...
    while (1) {
        ...
        } else if (hash == set->nodes[i]->_hash && set->nodes[i]->_key == key) {
            *index = i;
            return SET_TRUE;
        }
        ++i;
