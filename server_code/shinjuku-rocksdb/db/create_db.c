...
int main(int argc, char **argv) {
  ...
  rocksdb_put(db, writeoptions, long_key, strlen(long_key), long_value, 1000,
                    &err);
  assert(!err);
  ...
  size_t len;
  char new_long_key[8];
  snprintf(new_long_key, 8, "long_key");
  char * new_long_value = rocksdb_get(db, readoptions, new_long_key, strlen(new_long_key), &len, &err);
  assert(!err);
  assert(len == 1000);
